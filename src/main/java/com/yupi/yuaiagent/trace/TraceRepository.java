...
public class TraceRepository {
    ...
    public void init() {
        try {
            ...
            loadFromFile();
            log.info("[trace] repository initialized, storage path: {}", storageFile.getAbsolutePath());
        } catch (Exception e) {
            log.error("[trace] failed to initialize repository", e);
        }
    }
    ...
    public ExecutionTrace save(ExecutionTrace trace) {
        ...
        try {
            ...
            saveToFile();
            log.debug("[trace] saved traceId={}", trace.getTraceId());
            return trace;
        } finally {
    ...
    public void enforceRetentionPolicy(String userId) {
        ...
        try {
            ...
            for (int i = 0; i < toRemove; i++) {
                ...
                log.debug("[trace] evicted oldest traceId={} for userId={}", oldest.getTraceId(), userId);
            }

            saveToFile();
            log.info("[trace] retention policy enforced for userId={}, removed {} traces", userId, toRemove);
        } finally {
            lock.writeLock().unlock();
        }
    }
    ...
    private void loadFromFile() {
        if (storageFile.exists() && storageFile.length() > 0) {
            try {
                ...
                log.info("[trace] loaded {} traces from file", loaded.size());
            } catch (IOException e) {
                log.error("[trace] failed to load traces file", e);
            }
        }
    }
    ...
    private void saveToFile() {
        try {
            ...
            log.error("[trace] failed to save traces file", e);
        }
    }
}
